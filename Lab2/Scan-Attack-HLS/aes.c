...
void Cipher(state_t* state, uint8_t RoundKey[240])
{
  ...
  for (round = 1; round < Nr; ++round)
  {
    ...
    //for(loop=0;loop<4;loop++)
    //  for(loop2=0;loop2<4;loop2++) {
    //    f[loop*4+loop2] = (*state)[loop][loop2];
    //    //printf("%02x",(*state)[loop][loop2]);
    //  }
    //}
  }
...
void Cipher2(state_t* state, uint8_t RoundKey[240])
{
  ...
  for(i=0,j=0;i<192;i++)
  {
      if(i<16)
      {
          quotient = i/4;
          remainder = i%4;
          fprintf(fp,"%02x ",(*state)[quotient][remainder]);
          //printf("%02x ",(*state)[quotient][remainder]);
      }
      else
      {
          fprintf(fp,"%02x ",RoundKey[j]);
  ...
  if ( fpp == NULL )
    ...
    else
    {
        for(i=0,j=0;i<49;i++)
        {
            ...
            {
                temp_state[j] = jq;
                //printf("%c",temp_state[j]);
                j++;
            }
        }
        //printf("\n\rThe first few values are: %c ",jq);
        //printf("\n%i", j);
        for(i=0,j=0;i<32;i++)
        {
            tpp = hexx(temp_state[i]);
            jpp = hexx(temp_state[i+1]);
            ggf[j] = (16*tpp + jpp);
            //printf("%02x",ggf[j]);
            j++;
            i++;
        ...
        for(i=0;i<16;i++)
        {
            quotient = i/4;
            remainder = i%4;
            (*state)[quotient][remainder] = ggf[i];
            //printf("%02x",(*state)[quotient][remainder]);
        }
        //printf("\n");
    }
...
void AES_ECB_encrypt2(struct AES_ctx *ctx, uint8_t* buf)
{
  // The next function call encrypts the PlainText with the Key using AES algorithm.
  Cipher2((state_t*)buf, ctx->RoundKey);
}
