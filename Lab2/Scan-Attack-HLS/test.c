...
static int test_encrypt_ecb(void)
{
    ...
    uint8_t f0[16],f1[16],f_xor[16],b,rk0[16],rk1[16],rk[16], in_check[16];
    int count_1;
    struct AES_ctx ctx;
    struct AES_ctx ctx_check;
    int loop,loop2,loop3,loop4;

    for(loop4=0;loop4<1;loop4++) {
            printf("\nloop4 started");
    ...
    for(loop3=0;loop3<16;loop3++) {
        for(loop=0;loop<127;loop++) {
            ...
            //}
            for(loop2=0;loop2<16;loop2++) {
              f_xor[loop2]=f0[loop2]^f1[loop2];
            }
            //printf("\nf_xor out:");
            for(loop2=0;loop2<16;loop2++) {
              count_1 = count_1 + count_bits(f_xor[loop2]);
            //  printf("%02x",f_xor[loop2]);
            }
            //printf("\n Number of 1s = %d",count_1);
            if(count_1==9) {
                b = 226;
                rk0[loop3]=b^(2*loop);
                rk1[loop3]=b^(2*loop+1);
                //printf("\n loop_number = %d",loop);
                break;
            } else if(count_1==12) {
                b = 242;
                ...
                rk0[loop3]=b^(2*loop);
                rk1[loop3]=b^(2*loop+1);
            //printf("\n loop_number = %d : b = %d",loop, b);
                break;
            }
        }
        printf("\nrk0[%d] = %x, rk1[%d] = %x",loop3, rk0[loop3], loop3, rk1[loop3]);
    }

    for(loop3=0;loop3<pow(2,16);loop3++) {
	if(loop3 & 0x1)
	    rk[0]=rk0[0];
	else
	    rk[0]=rk1[0];
	...
	loop=0;
	for(loop2=0;loop2<16;loop2++) {
	    //ctx_check.RoundKey[loop2] = rk[loop2];
	    //printf("%02X",rk[loop2]);
	    in_check[loop2] = 0;
	}
	//printf(" = RK \n");
	AES_init_ctx(&ctx_check, rk);
	AES_ECB_encrypt(&ctx_check,in_check);
	for(loop2=0;loop2<16;loop2++) {
	    in[loop2] = 0;
	}
	AES_ECB_encrypt(&ctx,in);
	for(loop2=0;loop2<16;loop2++) {
	    if(in[loop2]==in_check[loop2])
		loop++;
	    //printf("%02X",rk[loop2]);
	}
	if(loop==16) {
